...
public class CrossedWires {
	...
	public static HashSet<xy> makeHashSet(String[] strs) {
		HashSet<xy> locs = new HashSet<>();
		xy ptr = new xy(0, 0);
		locs.add(new xy(ptr.x, ptr.y));
		xy[] dirs = new xy[]{new xy(1, 0), new xy(-1, 0), new xy(0, 1), new xy(0, -1)};
		for (String s : strs) {
			xy cur = new xy(0,0);
			int mag = Integer.decode(s.substring(1));
			int idx = 0;
			if (s.charAt(0) == 'R') {
				idx = 0;
	...
	public static HashMap<xy, Integer> makeHashMap(String[] strs) {
		HashMap<xy, Integer> locs = new HashMap<xy, Integer>();
		xy ptr = new xy(0, 0);
		locs.put(new xy(ptr.x, ptr.y), 0);
		xy[] dirs = new xy[]{new xy(1, 0), new xy(-1, 0), new xy(0, 1), new xy(0, -1)};
		int dist = 0;
		for (String s : strs) {
			xy cur = new xy(0,0);
			...
			for (int i = 0; i < mag; i++) {
				...
				ptr.y += dirs[idx].y;
				dist++;
				locs.put(new xy(ptr.x, ptr.y), dist);
			}
		}
		return locs;
	}
...
//		}
		HashSet<xy> h1 = makeHashSet(in1);
		HashSet<xy> h2 = makeHashSet(in2);
		HashMap<xy, Integer> m1 = makeHashMap(in1);
		HashMap<xy, Integer> m2 = makeHashMap(in2);
		System.out.println(h1.size());
		int minSoFar = 10000000;
		for (xy m : h1) {
//			System.out.println(m.x + " " + m.y);
			if (h2.contains(m)) {
				int dist = m1.get(m) + m2.get(m);
//				System.out.println(dist);
				if (dist > 0 && dist < minSoFar) {
					minSoFar = dist;
				}
			}
		}
