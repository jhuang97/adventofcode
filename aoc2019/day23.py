...
class IntComp:
    ...
    def tick(self, iv2):
        ...
        if opcode == 1:
            ...
            # else:
            if self.use_in_buffer and len(self.in_buffer) > 0:
            ...
            #     print(pvals[0])
            self.idx=self.idx+2
            self.out_buffer.append(pvals[0])
...
if __name__ == '__main__':
    ...
    while not done:
        ...
        packet_count = 0
        for k in active_set:
            if len(ics[k].out_buffer) >= 3:
                packet_count += 1
                addr = ics[k].out_buffer[0]
                x = ics[k].out_buffer[1]
                y = ics[k].out_buffer[2]
                ics[k].out_buffer = ics[k].out_buffer[3:]
                from_addr.add(k)
                to_addr.add(addr)
                if addr >= 0 and addr < N:
                    ics[addr].in_buffer.append(x)
                    ics[addr].in_buffer.append(y)
                elif addr == 255:
                    if not part1done:
                        print('part 1 answer: %d' % y)
                        print('%d ticks in part 1' % tcount)
                        part1done = True
                    nat = (x, y)
        if tcount % print_wait == 0 and len(from_addr)>0 and len(to_addr)>0:
            print('ticks %d-%d, packets to %s from %s' % (tcount-print_wait,tcount, str(sorted(to_addr)), str(sorted(from_addr))))
            from_addr = set()
            to_addr = set()
        if packet_count == 0 and nat != None:
            all_idle = True
            for kk in active_set:
                if not(len(ics[kk].in_buffer) == 0 and ics[kk].idle_count > 2):
                    all_idle = False
            if all_idle:
                ics[0].in_buffer.append(nat[0])
                ics[0].in_buffer.append(nat[1])
                print('tick %d: NAT sent (%d, %d)' % (tcount, nat[0], nat[1]))
                if nat[1] == nat_yprev:
                    print('part 2 answer: %d' % nat[1])
                    done = True
                nat_yprev = nat[1]
                nat = None
        tcount += 1
        if tcount == 4000:
            active_set = [0, 3, 8, 26, 34, 39] # deduced from watching which computers actually send and receive packets in the long run
    print('%d ticks in total' % tcount)
